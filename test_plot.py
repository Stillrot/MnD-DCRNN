...
parser.add_argument('--checkpoint', type=str, default='outputs/checkpoints/Mag_net_00000300', help="checkpoint of autoencoders")
...
PE_dis = []
for i in range(len(target_dis)):
    PE_dis.append(predicted_dis[i] - target_dis[i])



fig, ax = plt.subplots()
ax.hist(PE_dis, bins=100, range=[-120, 120], color='r')
ax.set_title('Distance')
ax.set_xlabel('Prediction Errors (km)')
ax.set_ylabel('Frequency')

if 'train' in dir_name:
    fig.text(0, 0, "%.4f" % (dis_loss))
    fig.savefig(os.path.join(opts.save_dir, opts.checkpoint[-6:] + '_train_PE_dis.png'))

else:
    fig.text(0, 0, "%.4f %.4f" % (np.mean(np.array(PE_dis)), np.std(np.array(PE_dis))))
    fig.savefig(os.path.join(opts.save_dir, opts.checkpoint[-6:] + '_test_PE_dis.png'))

print(np.mean(np.array(PE_dis)), np.std(np.array(PE_dis)))
...
for i in range(len(target_mag)):
    PE_mag.append(predicted_mag[i] - target_mag[i])


fig, ax = plt.subplots()
ax.hist(PE_mag, bins=100, range=[-7, 7], color='r')
ax.set_title('Magnitude')
ax.set_xlabel('Prediction Errors (magnitude)')
ax.set_ylabel('Frequency')


if 'train' in dir_name:
    fig.text(0, 0, "%.4f" % (mag_loss))
    fig.savefig(os.path.join(opts.save_dir, opts.checkpoint[-6:] + '_train_PE_mag.png'))

else:
    fig.text(0, 0, "%.4f %.4f" % (np.mean(np.array(PE_mag)), np.std(np.array(PE_mag))))
    fig.savefig(os.path.join(opts.save_dir, opts.checkpoint[-6:] + '_test_PE_mag.png'))
